...
class Recommender(abc.ABC):
    ...
    @abc.abstractmethod
    def predict(self, user: int, movie: int, timestamp: int) -> float:
        """
        :param user: User identifier
        :param movie: Movie identifier
        :param timestamp: Rating timestamp
        :return: Predicted rating of the user for the movie
        """
        raise NotImplementedError()

    def rmse(self, true_ratings) -> float:
        user_col = true_ratings["userId"].tolist()
        movie_col = true_ratings["movieId"].tolist()
        rating_col = true_ratings["rating"].tolist()
        timestamp_col = true_ratings["timestamp"].tolist()

        sum_rmse = 0
        for idx in range(len(user_col)):
            sum_rmse += (rating_col[idx] - self.predict(int(user_col[idx]), int(movie_col[idx]),
...
class NeighborhoodRecommender(Recommender):
    def initialize_predictor(self, ratings: pd.DataFrame):
        ...
        for user_idx in unique_users:
            r_user = ratings[ratings['userId'] == user_idx]['rating']
            avg_user = sum(r_user) / len(r_user)
            self.b_u_dict[user_idx] = avg_user - self.r_matrix_avg

        unique_movies = ratings['movieId'].unique()
        max_movie = int(max(unique_movies)) + 1
        for movie_idx in unique_movies:
            r_movie = list(ratings[ratings['movieId'] == movie_idx]['rating'])
            avg_movie = sum(r_movie) / len(r_movie)
            self.b_m_dict[movie_idx] = avg_movie - self.r_matrix_avg

        # similarity part

        self.r_wave_matrix = np.zeros((max_user, max_movie))
        for idx, row in ratings.iterrows():
            self.r_wave_matrix[int(row['userId'])][int(row['movieId'])] = row['rating'] - self.r_matrix_avg

        for user_idx in unique_users:
            self.similarity_dict[(user_idx, user_idx)] = 1
            for other_user_idx in unique_users:
                if user_idx > other_user_idx:
                    user1_movies = np.where(self.r_wave_matrix[int(user_idx)] != 0)[0]
                    user2_movies = np.where(self.r_wave_matrix[int(other_user_idx)] != 0)[0]
                    mutual_movies = np.intersect1d(user1_movies, user2_movies, assume_unique=True)
                    if len(mutual_movies) == 0:
                        self.similarity_dict[(user_idx, other_user_idx)] = 0
                        self.similarity_dict[(other_user_idx, user_idx)] = 0
                        continue
                    user_1_rating_consider = self.r_wave_matrix[int(user_idx)][mutual_movies]
                    user_2_rating_consider = self.r_wave_matrix[int(other_user_idx)][mutual_movies]
    ...
    def predict(self, user: int, movie: int, timestamp: int) -> float:
        ...
        :return: Predicted rating of the user for the movie
        """
        potential_users = self.r_matrix[(self.r_matrix.userId != user) & (self.r_matrix.movieId == movie)]['userId'].unique()
        ...
        for similarity, user_idx in top_k_lst:
            r_wave_other_user = self.r_wave_matrix[int(user_idx)][int(movie)]
        ...
        self.b_m_dict[movie] = self.b_m_dict[movie] if movie in self.b_m_dict else 0

        predicted_rating = self.r_matrix_avg + self.b_u_dict[user] + self.b_m_dict[movie] + corr_val
...
class LSRecommender(Recommender):
    def initialize_predictor(self, ratings: pd.DataFrame):
        self.n_users = int(max(ratings['userId']) + 1)
        n_movies = int(max(ratings['movieId']) + 1)
        n_records = len(ratings['movieId'])
        self.r_matrix = np.zeros((n_records, self.n_users + n_movies + 3))
        for idx, row in ratings.iterrows():
            curr_user = int(row['userId'])
            curr_movie = int(row['movieId'])
            curr_timestamp = datetime.datetime.fromtimestamp(row['timestamp'])
            self.r_matrix[idx][curr_user] = 1
            self.r_matrix[idx][self.n_users + curr_movie] = 1
    ...
    def predict(self, user: int, movie: int, timestamp: int) -> float:
        ...
        pred = self.r_matrix_avg + self.sol[int(user)] + self.sol[int(self.n_users + movie)] + b_w_indicator * self.sol[
...
class MFRecommender(Recommender):
    ...
    def initialize_predictor(self, ratings):
        n_users = int(max(ratings['userId']) + 1)
        n_movies = int(max(ratings['movieId']) + 1)
        self.b_u = np.zeros(n_users)
        self.b_m = np.zeros(n_movies)

        for user_idx in ratings['userId'].unique():
            r_user = list(ratings[ratings['userId'] == user_idx]['rating'])
            avg_user = sum(r_user) / len(r_user)
            self.b_u[int(user_idx)] = avg_user - self.r_matrix_avg

        for movie_idx in ratings['movieId'].unique():
            r_movie = list(ratings[ratings['movieId'] == movie_idx]['rating'])
            avg_movie = sum(r_movie) / len(r_movie)
            self.b_m[int(movie_idx)] = avg_movie - self.r_matrix_avg

        self.r_matrix = np.zeros((n_users, n_movies))
        for idx, row in ratings.iterrows():
            curr_user = int(row['userId'])
            curr_movie = int(row['movieId'])
            self.r_matrix[curr_user][curr_movie] = float(row['rating'])

        # Initialize user and movie latent feature matrice
        self.P = np.random.normal(scale=1. / self.K, size=(n_users, self.K))
        self.Q = np.random.normal(scale=1. / self.K, size=(n_movies, self.K))
    ...
    def sgd(self):
        ...
        for user, movie, rating in self.samples:
            ...
            prediction = self.predict(user, movie, 0)
            error = rating - prediction

            # Update biases
            self.b_u[user] += self.alpha * (error - self.beta * self.b_u[user])
            self.b_m[movie] += self.alpha * (error - self.beta * self.b_m[movie])
    ...
    def predict(self, user: int, movie: int, timestamp: int) -> float:
        ...
        :return: Predicted rating of the user for the movie
        """
        predicted_rating = self.r_matrix_avg + self.b_u[int(user)] + self.b_m[int(movie)] + self.P[int(user), :].dot(
            self.Q[int(movie), :].T)
        return predicted_rating if 0.5 <= predicted_rating <= 5 else 0.5 if predicted_rating < 0.5 else 5
...
class HybridMFRecommender(Recommender):
    def __init__(self, R, K, alpha, beta, iterations, movies_metadata):
        """
               Perform matrix factorization to predict empty
               entries in a matrix.
               Arguments
    ...
    def initialize_predictor(self, ratings):
        ...
        self.b_company = np.zeros(n_movies)
        self.b_genre = np.zeros(n_movies)
        self.b_country = np.zeros(n_movies)
        self.b_spoken = np.zeros(n_movies)
        self.b_runtime = np.zeros(n_movies)
        self.b_revenue = np.zeros(n_movies)
        self.b_budget = np.zeros(n_movies)
        self.b_popularity = np.zeros(n_movies)
        self.b_vote_count = np.zeros(n_movies)
        self.b_vote_avg = np.zeros(n_movies)
        self.b_years = np.zeros(n_movies)
    ...
    def predict(self, user: int, movie: int, original_movie=0) -> float:
        """
        :param original_movie:
        :param user: User identifier
        :param movie: Movie identifier
        :return: Predicted rating of the user for the movie
        """
        predicted_rating = self.r_matrix_avg + self.b_u[int(user)] + self.b_m[int(movie)] + self.b_company[movie] + \
                           self.b_genre[movie] + self.b_spoken[movie] + self.b_runtime[movie] + self.b_revenue[movie] + \
                           self.b_budget[movie] + self.b_popularity[movie] + self.b_vote_count[movie] + self.b_vote_avg[
                               movie] + self.b_years[movie] + self.P[int(user), :].dot(self.Q[int(movie), :].T)
        return predicted_rating if 0.5 <= predicted_rating <= 5 else 0.5 if predicted_rating < 0.5 else 5
